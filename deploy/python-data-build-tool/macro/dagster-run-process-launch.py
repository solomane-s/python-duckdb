...
try:
    ...
    with open(LOG_FILE, "w") as log:
        process = subprocess.Popen(
            ["nohup", "dagster", "dev", "-f", str(DAGSTER_FILE), "--port", "3000"],
            stdout=log,
    ...
    try:
        import webbrowser
        webbrowser.open("http://127.0.0.1:3000/asset-groups")
        print("✓ Interface Dagster ouverte dans le navigateur")
        print("   Pour ouvrir dans VS Code: Cmd+Shift+P > 'Simple Browser: Show'")
        print(f"   puis entrez: http://127.0.0.1:3000/asset-groups")
    except Exception as e:
        print(f"Note: Impossible d'ouvrir le navigateur automatiquement: {e}")
        print(f"Ouvrez manuellement: http://127.0.0.1:3000/asset-groups")
